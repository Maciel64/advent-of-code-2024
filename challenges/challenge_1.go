...
func Challenge_1() {
	file, _ := os.Open("./inputs/input_1.txt")
	...
	for _, line := range lines {
		...
		if len(parts) < 2 {
			fmt.Println("Linha inválida, ignorando:", line)
			continue
		}
...
func Challenge_1_part_2() {
	...
	for _, leftValue := range leftNumbers {
		for _, rightValue := range rightNumbers {
			if leftValue == rightValue {
				similarity += rightValue
			}
		}
	}

	fmt.Println("Similaridade total: ", similarity)
}
